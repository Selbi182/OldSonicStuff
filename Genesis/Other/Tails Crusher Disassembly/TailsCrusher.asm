...
MainLoop:
		...
		move.l	#(($60000003+$A800000)+$1C0000),d2
		move.l	d2,($C00004).l

Loop_LoadNormalPose:
		move.l	d4,-(sp)		; backup d4
		moveq	#10,d4			; set number of loops (+1)
		bsr.w	Sub_WriteArt		; go to write art to VDP subroutine
		...
		move.l	#5,d0
		move.w	#$8F02,($C00004).l
		move.l	#(($60000003+$14000000)+$240000),d2
		move.l	d2,($C00004).l

Loop_LoadEggmanKnuckles:
		move.l	d4,-(sp)		; backup d4
		moveq	#7,d4			; set number of loops (+1)
		bsr.w	Sub_WriteArt		; go to write art to VDP subroutine
		move.l	(sp)+,d4		; restore d4
		addi.l	#$800000,d2
		move.l	d2,($C00004).l
...
NoSonicTailsChangePos:
		move.w	#$38,d5
		bsr.w	DelaySystem
		lea	(Pal1_Default).w,a6	; load main palette
		move.w	#$8164,($C00004).l
		move.l	#$40,d6			; size of palette
		bsr.w	Sub_PalLoad		; load palette
		move.w	#$8E,d5			; number of frames to wait
		bsr.w	DelaySystem		; don't do anything $8E frames long

		lea	(Sound2_Crush).w,a5	; load crushing sound
		bsr.w	PlaySound		; play that sound
		move.w	#$12,d5
		bsr.w	DelaySystem
		move.l	#$95,d3
		moveq	#4,d0
...
Loop_SonicLookingUp:
		move.l	d4,-(sp)		; backup d4
		moveq	#4,d4			; set number of loops (+1)
		bsr.w	Sub_WriteArt2		; go to write art to VDP subroutine d3 version)
		move.l	(sp)+,d4		; restore d4
		addi.l	#$800000,d2
		move.l	d2,($C00004).l
		...
		moveq	#5,d0
		move.w	#$8F02,($C00004).l
		move.l	#(($60000003+$A000000)+$260000),d2
		move.l	d2,($C00004).l
...
Loop_WaitForTing:
		...
		andi.w	#$FFF8,d1
		move.w	d1,($C00000).l
		lea	(Sound1_Ting).w,a5	; load "ting" sound (when Sonic looks up)
		bsr.w	PlaySound		; play that sound
		tst.b	d7			; is alternate mode on?
		bne.w	LoadCyanScreen		; if yes, don't make any gore stuff
		move.l	#$C1,d1
		move.l	#4,d3
		lea	(Pal2_Crushing).w,a6	; load palette 2 (crushing palette) into a6  (For fun: To make the scene look even more creapier, comment out this line.)
		move.l	#5,d4
...
Loop_LoadBlood1:
		move.l	d4,-(sp)		; backup d4
		moveq	#17,d4			; set number of loops (+1)
		bsr.w	Sub_WriteArt		; go to write art to VDP subroutine
		move.l	(sp)+,d4		; restore d4
		addi.l	#$800000,d2
		move.l	d2,($C00004).l
		...
		move.l	#5,d0
		move.w	#$8F02,($C00004).l
		move.l	#(($60000003+$18800000)+$1C0000),d2
		move.l	d2,($C00004).l
...
Loop_LoadDrippingBlood:
		...
		move.l	#(($60000003+$19800000)+$1C0000),d2
		move.l	d2,($C00004).l
		move.l	d4,-(sp)		; backup d4
		moveq	#3,d4			; set number of loops (+1)
		bsr.w	Sub_WriteArt		; go to write art to VDP subroutine
		move.l	(sp)+,d4		; restore d4
		move.w	d4,d5
		bsr.w	DelaySystem
...
loc_916:
		...
		suba.l	#$80,a6
		move.l	#1,d6
		bsr.w	Sub_PalFadeOut		; fade out palette
		lea	(Pal3_FadeOut).w,a6	; fade out Sonic/Tails first, then Eggman/Knuckles/3
		move.l	#0,d6
		bsr.w	Sub_PalFadeOut
		bra.s	NoFadeOut
...
LoadCyanScreen:						; Offset: $094C
		move.w	#$10,d5
		bsr.w	DelaySystem
		lea	(Pal1_Default).w,a6	; load default palette (probably totally unnecesary since the real palette gets loaded a few lines below)
		move.l	#0,d6
		bsr.w	Sub_PalFadeOut

NoFadeOut:
		bsr.w	Sub_ResetVDP
		bsr.w	Sub_LoadCyanArt
		move.l	#1,d1
		move.l	#8,d0
		move.w	#$8F02,($C00004).l
		move.l	#(($60000003+$4800000)+$40000),d2
		move.l	d2,($C00004).l
...
loc_9E0:
		...
		bsr.w	DelaySystem
		move.w	#$8164,($C00004).l
		lea	(Pal4_Cyan1).w,a6	; load Cyan's palette 1. It's jumping between palette 1 and 2.
		move.l	#$40,d6
		bsr.w	Sub_PalLoad
		bsr.w	PlaySound_2
...
Loop_DisplayCyan:
		...
		move.w	#$A0,d5
		bsr.w	DelaySystem
		bra.w	MainLoop		; Loop the entire game. If you'd put an rts in here, the game would just stay black after the Cyan screen.
...
PlaySound_4:						; Offset: $0B22
		...
		movem.l	d3-d4,-(sp)		; backup d3 and d4
		moveq	#11,d3			; set number of loops
		moveq	#$50,d4			; set starting value to $50
...
PlaySound_Loop:
		...
		move.b	(a5)+,($A04001).l
		bsr.s	PlaySound_3
		addq.b	#4,d4			; increase number to be written into ($A04000).l by 4
		dbf	d3,PlaySound_Loop	; repeat

		move.b	d4,($A04000).l		; the last one works...
		bsr.s	PlaySound_3		; ...a little different
		movem.l	(sp)+,d3-d4		; restore d3 and d4
...
Sub_DumpPalRestore:					; Offset: $0DFC
		move.l	d6,-(sp)
		move.w	#$8F02,($C00004).l
...
Loop_E16:
		...
		move.l	(sp)+,d6
		rts
; End of function Sub_DumpPalRestore
...
loc_F6A:
		...
		movem.l	(sp)+,d0-a2
		rts
; End of function Sub_PalFadeOut
...
Sub_LoadMainArt:					; Offset: $10DE
		movem.l	d0-a1,-(sp)
		lea	(Art1_Normal).w,a0
		move.l	#$683C,d0
		subi.l	#$1690,d0
...
loc_1180:
		...
		movem.l	(sp)+,d0-a1
		rts
; End of function Sub_LoadMainArt
...
Sub_LoadCyanArt:					; Offset: $118C
		movem.l	d0-a1,-(sp)
		lea	(Art2_Cyan).w,a0
		move.l	#$8588,d0
		subi.l	#$683C,d0
		bra.w	loc_10F6
; End of function Sub_LoadCyanArt
...
; End of function DelaySystem_Helper
...
; ===========================================================================
; ---------------------------------------------------------------------------
; Custom subroutine to reduce the number of those art loading thingies.
; ---------------------------------------------------------------------------

Sub_WriteArt:
		subq.w	#1,d4		; to make things easier to count, you don't need to substract 1 all the time yourself

WA_Loop:
		move.w	d1,($C00000).l	; write tiles to VDP from d1
		addq.w	#1,d1		; increase value to d1
		dbf	d4,WA_Loop	; repeat X times
		rts			; return
; End of function Sub_WriteArt

; ---------------------------------------------------------------------------
; This is basically the same routine, except that it's dealing with d3,
; rather than with d1.
; ---------------------------------------------------------------------------

Sub_WriteArt2:
		subq.w	#1,d4		; to make things easier to count, the number of loops is the number of this code below

WA2_Loop:
		move.w	d3,($C00000).l	; write tiles to VDP from d3
		addq.w	#1,d3		; increase value to d3
		dbf	d4,WA2_Loop	; repeat X times
		rts			; return
; End of function Sub_WriteArt2
; ===========================================================================
...
; Optimized: Because of this giant space taking, I disabled it. You will only
;            need it on real hardware and some emulators.
; ---------------------------------------------------------------------------
